...
def send_beam(contraption, beam, dir):
    beams = [beam]
    dirs = [dir]
    visited = set()
...
for row in range(height):
    max_energy = max(max_energy, send_beam(contraption, [row, 0], [0, 1]))
    max_energy = max(max_energy, send_beam(contraption, [row, width - 1], [0, -1]))

for col in range(width):
    max_energy = max(max_energy, send_beam(contraption, [0, col], [1, 0]))
    max_energy = max(max_energy, send_beam(contraption, [height - 1, col], [-1, 0]))
